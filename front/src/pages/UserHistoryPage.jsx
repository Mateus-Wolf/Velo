...
export default function UserHistoryPage() {
    ...
    const filtered = useMemo(() => {
        ...
        if (filterWorkplace) list = list.filter((a) => String(a.workplace_id) === filterWorkplace);
        ...
        return list;
    }, [appointments, filterStatus, filterWorkplace, dateFrom, dateTo, searchClient, filterPayment, sortRating, sortValue]);

    const hasActiveFilters = filterStatus !== 'all' || filterWorkplace || dateFrom || dateTo || searchClient.trim() || filterPayment !== 'all' || sortRating !== 'none' || sortValue !== 'none';
        ...
        <div className="min-h-screen gradient-bg">
                ...
                <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">{t('history.calendarBtn', 'Agenda')}</span>
                        </motion.button>
                        <UserDropdown />
                    </div>
                </div>
            ...
            <main className="relative z-10 mx-auto max-w-4xl px-4 py-8 sm:px-6">
                ...
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
                    <h1 className="text-2xl font-bold text-surface-50 sm:text-3xl">{t('history.userHistoryTitle', 'Meu Histórico')}</h1>
                    <p className="mt-1 text-sm text-surface-200/50">
                        {t('history.userHistoryDesc', 'Todos os seus atendimentos em todos os locais')}
                    </p>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-6">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        {hasActiveFilters && (
                            <button
                                onClick={() => { 
                                    setFilterStatus('all'); setFilterWorkplace(''); setDateFrom(''); setDateTo(''); 
                                    setSearchClient(''); setFilterPayment('all'); setSortRating('none'); setSortValue('none'); 
                                }}
                        ...
                        {showFilters && (
                            ...
                            >
                                <div className="glass rounded-xl p-4 space-y-4 mb-4">
                                    ...
                                    {/* Workplace filter */}
                                    <div>
                                        <label className="text-xs text-surface-200/50 mb-1 block">{t('history.workplaceFilter', 'Local de trabalho')}</label>
                                        <select
                                        ...
                                        >
                                            <option value="" className="bg-surface-900">{t('history.allWorkplaces', 'Todos os locais')}</option>
                                            {workplaces.map((wp) => (
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs text-surface-200/50 mb-1 block">Data início</label>
                                            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
                                                className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-surface-50 outline-none focus:border-brand-500" />
                                        </div>
                                        <div>
                                            <label className="text-xs text-surface-200/50 mb-1 block">Data fim</label>

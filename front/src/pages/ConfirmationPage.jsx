...
export default function ConfirmationPage() {
    ...
    useEffect(() => {
        if (!token) {
            setStatus('error');
            setMessage(t('confirmation.invalidLink', 'Link inválido. Nenhum token de confirmação foi fornecido.'));
            return;
        }

        const confirmAppointment = async () => {
            try {
                // Remove /api v1 prefix since public endpoint might be outside or inside api v1
                // Assuming it's in /api/v1/public/appointments
                const { data } = await api.post(`/public/appointments/confirm?token=${token}`);
                setStatus('success');
                setMessage(data.message || t('confirmation.successMsg', 'Agendamento confirmado com sucesso!'));
            } catch (err) {
                setStatus('error');
                setMessage(err.response?.data?.detail || t('confirmation.errorMsg', 'Ocorreu um erro ao confirmar seu agendamento. Talvez o link tenha expirado.'));
            }
        };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-surface-950 font-sans relative overflow-hidden">
            ...
            <motion.div
                ...
                className="w-full max-w-md bg-surface-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 text-center relative z-10 shadow-2xl shadow-emerald-900/20"
            >
                ...
                <h1 className="text-2xl font-bold text-surface-50 mb-3">
                    {status === 'loading' ? t('confirmation.confirming', 'Confirmando...') : status === 'success' ? t('confirmation.allGood', 'Tudo certo!') : t('confirmation.oops', 'Ops!')}
                </h1>

                <p className="text-surface-200/70 leading-relaxed mb-8">
                    {status === 'loading' ? t('confirmation.loadingDesc', 'Aguarde um momento enquanto validamos seu agendamento.') : message}
                </p>

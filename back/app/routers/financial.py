...
def get_financial_goal_history(
    ...
    try:
        history = get_goal_history(db=db, user_id=current_user.id)
...
def delete_financial_goal_history(
    ...
    try:
        count = clear_goal_history(db=db, user_id=current_user.id)
...
def get_financial_projection(
    ...
    try:
        user_goal = current_user.monthly_goal or 0.0
        if user_goal <= 0:
